...
//            if ($row + 1 > 65536 or $column + 1 > 256) {
    ...
    /**
     * Insert a 24bit bitmap img in a worksheet.
     *
     * @access public
     * @param integer $row     The row we are going to insert the bitmap into
     * @param integer $col     The column we are going to insert the bitmap into
     * @param mixed   $bitmap  The bitmap filename or GD-img resource
     * @param integer $x       The horizontal position (offset) of the img inside the cell.
     * @param integer $y       The vertical position (offset) of the img inside the cell.
    ...
    public function insertBitmap($row, $col, $bitmap, $x = 0, $y = 0, $scale_x = 1, $scale_y = 1)
    {
        ...
        // Scale the frame of the img.
        $width  *= $scale_x;
        $height *= $scale_y;

        // Calculate the vertices of the img and write the OBJ record
    ...
    /**
     * Calculate the vertices that define the position of the img as required by
     * the OBJ record.
     *
     *         +------------+------------+
     *         |     A      |      B     |
     ...
     * @param integer $width     Width of img frame
     * @param integer $height    Height of img frame
     */
    public function positionImage($col_start, $row_start, $x1, $y1, $width, $height)
    {
        ...
        // Subtract the underlying cell widths to find the end cell of the img
        ...
        // Subtract the underlying cell heights to find the end cell of the img
    ...
    /**
     * Convert a GD-img into the internal format.
     *
     * @access private
     * @param resource $image The img to process
    ...
    public function processBitmap($bitmap)
    {
        ...
        if ($identity['ident'] != "BM") {
            throw new PHPExcel_Writer_Exception("$bitmap doesn't appear to be a valid bitmap img.\n");
        }
        ...
        $data   = substr($data, 8);
        if ($width > 0xFFFF) {
            throw new PHPExcel_Writer_Exception("$bitmap: largest img width supported is 65k.\n");
        }
        if ($height > 0xFFFF) {
            throw new PHPExcel_Writer_Exception("$bitmap: largest img height supported is 65k.\n");
        }
        ...
        if ($planes_and_bitcount[1] != 1) {
            throw new PHPExcel_Writer_Exception("$bitmap: only 1 plane supported in bitmap img.\n");
        }
        ...
        if ($compression['comp'] != 0) {
            throw new PHPExcel_Writer_Exception("$bitmap: compression not supported in bitmap img.\n");
        }
